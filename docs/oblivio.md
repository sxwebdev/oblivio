...
| Утечка через swap / coredump (server)             | `memguard` для серверных секретов; на хосте — отключить swap или включить swap-encryption              |
...
| Phishing / replay логина                          | WebAuthn рекомендуется как обязательный 2-й фактор для прод-аккаунтов                                  |
| Auto-lock                                         | По бездействию (`visibilitychange`/таймер), `beforeunload`, ручной lock                                |
| Tamper / rollback на сервере                      | Audit-chain в Postgres (см. §6.7); клиент при desync сравнивает локальный last-known root с серверным  |
...
| ------------------------------------ | --------- | ---------------- | ----------- |
| Client `master_key` (per-user из БД) | 3         | 131072 (128 MiB) | 4           |
| Server `argon2(auth_key)`            | 3         | 65536 (64 MiB)   | 1           |
...
4. Показывает recovery_code пользователю **ровно один раз** (PDF + копия в clipboard
   с предупреждением о том, что это последняя возможность).
...
В каждом ciphertext envelope первый байт — `version`. Текущая версия `0x01`
(AES-GCM + Argon2id + двухстадийное KDF). Будущие версии могут добавить
XChaCha20-Poly1305 / OPAQUE / post-quantum. Старые версии расшифровываются как было.
...
**Альтернатива:** делать TOTP для login полностью на клиенте с PAKE-handshake.
В MVP оставляем server-side TOTP с derived key — это компромисс между ZK и удобством,
но фундаментально безопасен (secret недоступен серверу без auth_key, который сервер не хранит).
...
Recovery не перешифровывает записи — только обёртку `vault_key`.
...
Cookies / headers: префикс `__Host-Auth=`, `HttpOnly`, `Secure`, `SameSite=Strict`,
`Path=/`. Для multi-client (mobile/extension) Bearer header — основной механизм.
...
`self_hash = SHA-256(prev_hash || row_canonical_json)`. Сервер хранит последний
`self_hash` в `system_state.audit_chain_head`. Раз в сутки фоновый job пересчитывает
цепочку и сравнивает с кешированным head — alarm при mismatch.
...
Перед каждым запросом репозиторий выполняет
`SET LOCAL app.current_user_id = $1` через `pgx.QueryExecMode`.

### 6.8 Crypto-shredding при удалении аккаунта

Все удаления — **физические**, без `deleted_at`. `DELETE FROM users WHERE id = $1`
каскадно удаляет vault, проекты, записи, сессии, audit-log пользователя. Поскольку
сервер не имеет ключей расшифровки и `wrapped_vault_key` исчезает вместе с
`user_vault`, ciphertext в любых сохранившихся бэкапах становится нечитаем.
Когда retention бэкапов истекает (например, 90 дней), все следы пользователя
исчезают окончательно — это и есть GDPR-совместимый crypto-shred.
...
- **Rate limiting**: per-IP и per-email на `Authorize`, `GetKDFParams`,
  `GetRecoveryParams`, `RecoveryStart`. Token bucket в таблице `rate_limit_buckets`.
...
- **Prevent enumeration**: `GetKDFParams` для несуществующих email возвращает
  стабильные псевдо-параметры (HMAC от email + server-secret), чтобы клиент-злоумышленник
  не отличал факт существования пользователя до этапа `Authorize`.
...
```go
type Secrets struct {
    accessKey  *memguard.LockedBuffer  // JWT access signing key
    refreshKey *memguard.LockedBuffer  // JWT refresh signing key
    vaultToken *memguard.LockedBuffer  // HashiCorp Vault token (если используем напрямую)
}
```

Загрузка: при старте — `memguard.NewBufferFromBytes(rawKey)`, потом `Destroy` после
ротации. Все операции, требующие ключ, делаются под `key.Bytes()` и сразу же освобождаются.

`memguard.CatchInterrupt()` — чистит память при SIGINT/SIGTERM.
...
`internal/api/middleware/security_headers.go` ставит на все ответы (включая static):
...
`'unsafe-inline'` для `style-src` — компромисс на старте (Tailwind 4 inline atomic
styles иногда требуются), позже — Trusted Types + nonces.
...
## 17. Open questions (можно отложить)
