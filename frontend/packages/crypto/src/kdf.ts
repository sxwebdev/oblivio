...
export async function hkdfSha256(
  ...
  const bits = await crypto.subtle.deriveBits(
    { name: "HKDF", hash: "SHA-256", info: utf8(info) as unknown as ArrayBuffer, salt: salt as unknown as ArrayBuffer },
    ikmKey,
    length * 8,
  );
...
export async function deriveLoginTotpKey(
  ...
  const raw = await hkdfSha256(authKey, HKDF_LOGIN_TOTP_INFO, new Uint8Array(0), 32);
  return crypto.subtle.importKey(
    "raw",
    raw as unknown as ArrayBuffer,
    { name: "AES-GCM" },
